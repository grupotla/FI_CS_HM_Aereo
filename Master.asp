...
if CountTableValues >= 0 then
	...
	Tax = aTableValues(11, 0)
	isConsigneer = CheckNum(aTableValues(12, 0))
	isShipper = CheckNum(aTableValues(13, 0))
	BusinessGID = CheckNum(aTableValues(14, 0))
	UserCreate = CheckNum(aTableValues(15, 0))
	UserModify = CheckNum(aTableValues(16, 0))
	CreatedIn = aTableValues(17, 0)
else
	...
	<INPUT name="AttnID" type=hidden value="<%=AttnID%>">
		...
		<TR><TD class=label align=right><b>Creado En:</b></TD><TD class=label align=left colspan=2>
			...
			<select name="CreatedIn" id="Pais donde el registro es creado" class="label">
				<option value="-1">Seleccionar</option>
				<%DisplayCountries CreatedIn%>
			</select>	
			<%else%>
				<%=TranslateCountry(CreatedIn)%>
				<input type="hidden" name="CreatedIn" value="<%=Mid(Session("Countries"),3,2)%>">
			<%end if%>
		</TD></TR>
		...
		<TD colspan="2" class=label align=center>
			...
			<%else%>
					 ...
					 select case GroupID
					 case 7%>									 
					 <TD class=label align=center colspan=2><INPUT name=enviar type=button onClick="top.opener.document.forms[0].AccountConsignerNo.value = '<%=AccountNo%>';top.opener.document.forms[0].ConsignerData.value = '<%=Name%>\n<%=Address%>\n<%=Phone1%><%if Phone2 <> "" then response.Write " \" & Phone2 end if%><%if Attn <> "" then response.Write "\nATTN: " & Attn end if%>';top.opener.document.forms[0].ConsignerID.value=<%=ObjectID%>;top.close();" value="&nbsp;&nbsp;Asignar&nbsp;&nbsp;" class=label></TD>
					 <%case 10%>
					 <TD class=label align=center colspan=2><INPUT name=enviar type=button onClick="top.opener.document.forms[0].AccountShipperNo.value='<%=AccountNo%>';top.opener.document.forms[0].ShipperData.value='<%=Name%>\n<%=Address%>\n<%=Phone1%><%if Phone2 <> "" then response.Write " \" & Phone2 end if%>';top.opener.document.forms[0].ShipperID.value=<%=ObjectID%>;top.close();" value="&nbsp;&nbsp;Asignar&nbsp;&nbsp;" class=label></TD>
					 <%end select
					 end if%>
