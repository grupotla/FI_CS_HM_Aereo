...
CountList2Values = -1
	if ObjectID = 0 then
		...
		SQLQuery2 = "select RangeID, Val from Ranges where Expired = 0 order by Val Asc"
	else
		SQLQuery = "select CarrierID, Name, Countries from Carriers where Expired = 0 and CarrierID=" & CarrierID
		SQLQuery2 = "select RangeID, Val from Ranges where Expired = 0 and RangeID=" & RangeID
	End If
...
<SCRIPT LANGUAGE="JavaScript">
	function validar(Action) {
		...
		if (!valSelec(document.forma.RangeID)){return (false)};
		<%
		end if
		%>
...
<BODY text=#000000 vLink=#000000 aLink=#000000 link=#000000 bgColor=#ffffff leftMargin=0 topMargin=0 marginheight="0" marginwidth="0">
	...
	<INPUT name="CT" type=hidden value="<%=CreatedTime%>">
		...
		<TR><TD class=label align=right><b>Rango:</b></TD><TD class=label align=left>
		<%
		if ObjectID = 0 then
		%>
		<select name="RangeID" id="Rango" class=label>
		<option value="-1">Seleccionar</option>
		<%
		...
		%>
		<%=aList2Values(1,0)%>
		<input type="hidden" name="RangeID" id="Rango"  value="<%=aList2Values(0,0)%>">
    	<%
		End If
		%>
		</TD></TR> 
