    ...
    Function SendNotification(Status, SQLQuery, AwbType, Comentario, aList1Valuesi, SendShipper, Mode)
        ...
        if CountListaMails > -1 then        
            ...
            for i=0 to CountListaMails
                ...
                'if Mode = 1 then 'Mode es 1 cuando viene de php                    
                '    response.write (  ListaMails(1,i) & " " & ListaMails(8,i) & " " &  ListaMails(9,i) & "\n\n" )			
                'end if	
    ...
    'comentado temporalmente solicitado por Ticket#2015062404000623 — Fwd: Re: NUEVO SISTEMA DE AUTO NOTIFICACIONES AIMAR Linea 6
                                        ...
                                        'if Countries <> "GT" then
                                            if Left(Countriesi, 2) = "BZ" then
                                                result = SendMails(Body_b, EmailStr, subject, from)
                                            else
                                                result = SendMails(BodySpanish, EmailStr, subject, from)
                                            end if
                                        'end if
                                    end if
                                else
            ...
            for i=0 to CountListaMails		
                ...
                if ListaMails(8,i) = "Desarrollo" and ListaMails(9,i) = "Si" then 'Copia
                    ...
                    result = SendMails(BodySpanish & eSendYes & eSendYesAge & Divisiones, ListaMails(1,i), subject, from)
                    ...
                    if result <> -1 then
                        ...
                        if Mode = 1 then
                            ...
                            response.write "<span style='color:green'>Autonotificacion enviada correctamente.<span><br>" 
                        end if

                    else
                                                
                        if Mode = 1 then                            
                            response.write "Problemas para enviar emails."
                        else
                            response.write "<span style='color:orange'>Problemas para enviar emails.<span><br>"                             
                        end if
                        ...
                        if UCase(ListaMails(1,i)) <> UCase(emailAnt) then
                            if ListaMails(10,i) = "Si" then 'Rechazo
                                if ListaMails(8,i) = "Desarrollo" then 
                                    result = SendMails(BodyNoEnvio, ListaMails(1,i), "NO SE ENVIO:" & subject, from)
                                else
                                    if local_ = 0 then
                                        result = SendMails(BodyNoEnvio, ListaMails(1,i), "NO SE ENVIO:" & subject, from)
                                    end if
