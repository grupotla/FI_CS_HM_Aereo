...
Dim ArrAwbType, aTableValues, CountTableValues, aList6Values, CountList6Values, CantItems, Movimiento, homos
Dim Msg, facturacion, ItemsPedidos, esquema, PedidoCliente, PedidoRubro
Dim ConsignerID, ConsignerData, Countries, AWBNumber, HAWBNumber
Dim Conn, rs, QuerySelect, TipoCarga, ObjectID, AwbType, i, Currencies, result, iLink, orden
...
CantItems = 10
    ...
    QuerySelect = "SELECT AwbID, CreatedDate, CreatedTime, AwbNumber, HAwbNumber, Countries, ConsignerData, TotNoOfPieces, TotWeight, Routing, CarrierID, AirportDepID, AirportDesID, SalespersonID, ShipperData, AgentContactSignature, RoutingID, AgentData, ConsignerID FROM " & IIf(AWBType = "1","Awb","Awbi") & " WHERE AwbID = " & ObjectID
    'response.write QuerySelect & "<br>"
    Set rs = Conn.Execute(QuerySelect)
    If Not rs.EOF Then
        RoutingID = CheckNum(rs("RoutingID"))
        AWBNumber = rs("AwbNumber")
        HAWBNumber = rs("HAwbNumber")
        Countries = rs("Countries")
        ConsignerID = rs("ConsignerID")
        ConsignerData = rs("ConsignerData")
    End If
    CloseOBJ rs
    ...
    QuerySelect = "Select UserID, ItemName, ItemID, CurrencyID, Value, OverSold, Local, PrepaidCollect, ServiceID, ServiceName, InvoiceID, CalcInBL, DocType, '', '', 0, ChargeID, '', COALESCE(id_cliente,0), COALESCE(id_pedido,0), TRIM(COALESCE(pedido_erp,'')), COALESCE(cliente_nombre,''), COALESCE(Regimen,''), COALESCE(TarifaPricing,''), COALESCE(TarifaTipo,''), AgentTyp, Pos, '', '' FROM ChargeItems WHERE Expired=0 and AwbID=" & ObjectID & " and DocTyp=" & IIf(AWBType = 1,"0","1") & " ORDER BY " & orden 
    'response.write QuerySelect & "<br>"
    Set rs = Conn.Execute(QuerySelect)
    If Not rs.EOF Then
    ...
    CloseOBJs rs, Conn
        ...
        'response.write AWBNumber & " " & HAWBNumber & " " & Countries & "<br>"
    ...
    if TipoConta = "" then     
        response.write "<font family=verdana color=navy>No hay Tipo Conta definida para : " & Countries & "</font><br>" 
	end if

    if esquema = ""  then
        response.write "<font family=verdana color=navy>No hay Esquema definido para : " & Countries & "</font><br>" 
    end if
...
next
    ...
    "WHERE a.d1 = '" & Movimiento & "' AND a.d2 = 'A' AND a.id_servicio || '-' || a.id_rubro || '-' || a.d3 IN (" & homos & ") " & _ 
    "LIMIT 100"
    'response.write QuerySelect & "<br>"
...
<form name="forma2" action="AwbSaves.asp" method="post" target="operations" onsubmit="move();">
...
<form name="forma" action="AwbCharges.asp" method="post" onsubmit="move();">
	...
	<INPUT name="HAWBNumber"        type=hidden value="<%=HAWBNumber%>">
...
<table width="96%" border="0" cellpadding="2" cellspacing="0" align="center">
  ...
  <tr>
    <td colspan="4" align="center"  valign="top" style="border:0px" cellpadding=0 cellspacing=0>
		...
		<TR>
            <TH class=label align=right>AwbID&nbsp;:&nbsp;</TH>
            <TD class=label align=left><%=ObjectID%></TD>
        </TR> 
		<TR>
            <TH class=label align=right>AwbNumber&nbsp;:&nbsp;</TH>
            <TD class=label align=left><%=AWBNumber%></TD>
        </TR> 
		<TR>
            <TH class=label align=right>HAwbNumber&nbsp;:&nbsp;</TH>
            <TD class=label align=left><%=HAWBNumber%></TD>
        </TR> 
		<TR>
            <TH class=label align=right>Countries&nbsp;:&nbsp;</TH>
            <TD class=label align=left><%=Countries%></TD>
        </TR> 
		<TR>
				...
				<TR>
                    <TD class=label align=center colspan=3>
                        ...
                        <TR>
                            ...
                            <button onClick="Javascript:GetData(24);return (false);" title="Buscar Cliente"><img src="img/Search16.png" /></button>
...
<script type="text/javascript">
    ...
    function Ordenamiento(tipo) {
        ...
        var url = 'AwbCharges.asp?OID=<%=ObjectID%>&TC=<%=TipoCarga%>&AT=<%=AwbType%>';
        var orden = "";
        switch (tipo) {
    ...
    function SearchCharge(ChargePos) {
        ...
        window.open('Search_Charges.asp?GID=18&OID=' + <%=ObjectID %> + '&C=' + '<%=Countries%>' + '&AT=<%=AWBType%>&N=' + ChargeName + '&NID=' + ChargePos + '&IL=' + (document.forma.elements['T' + ChargePos].value * 1 + 1) + '&CM=' + ChargeMoneda + '&No=' + iNo + '&ServiceID=' + servicio + '&ItemID=' + rubro + '&esquema=<%=esquema%>&impex=<%=Movimiento%>&TPI=' + CargoTipo + '&RegimenID=' + Regimen + '&TC=' + TC,'AWBData','height=300,width=450,menubar=0,resizable=1,scrollbars=1,toolbar=0');
    ...
    function DelCharge(Pos) {
	 ...
	 function SaveCharge(Pos) {
         ...
         var text = 'Confirme Grabar rubro asignado a cliente ' + document.forma.elements['ConsignerID'].value + ' ' + document.forma.elements['ConsignerData'].value.substring(1,30) + ' ?';
    ...
    function GetData(GID){
    	window.open('Search_AWBData.asp?GID=' + GID,'BLData','height=200,width=450,menubar=0,resizable=1,scrollbars=1,toolbar=0,status=1');
    }
    ...
    function Asignar(){
        ...
        if (confirm('Confirmar Asignar Cliente?')) {
            ...
            document.forma.action = "AwbSaves.asp";
            document.forma.submit();
    ...
    function ClienteProc(tipo) {
        ...
        if (confirm('Confirme ' + document.forma.ObservacionesErp.value + ' ?')) {
            ...
            document.forma.action = "AwbCharges.asp";
    ...
    //selecciona('forma.TC','<%=TipoCarga%>');
        ...
        alert('Verifique conectividad con server CS');
